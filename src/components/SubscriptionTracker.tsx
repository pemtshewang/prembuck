...
export function SubscriptionTracker() {
  const subscriptions = useAuraStore((state) => state.subscriptions);
  const currency = useAuraStore((state) => state.currency);
  const rates = useAuraStore((state) => state.rates);

  const totalBurn = subscriptions.reduce((acc, s) => acc + s.amount, 0);
    ...
    <section className="space-y-8 animate-in fade-in slide-in-from-right-8 duration-1000">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {subscriptions.map((sub) => {
          const Icon = iconMap[sub.type] || CreditCard;
          return (
            <div
              key={sub.id}
            ...
            >
              <div className="flex items-start justify-between relative z-10">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-on-surface-variant font-light">Renewal: {sub.renewal}</p>
                  </div>
                </div>
              ...
              <div className="flex items-end justify-between relative z-10">
                 <p className="text-xl font-bold font-mono tracking-tighter">{formatCurrency(sub.amount, currency, rates)}</p>
                 <span className={cn(
                   "px-2.5 py-1 rounded-full text-[8px] font-bold uppercase tracking-[0.1em] border bg-white/5 text-on-surface-variant border-white/10",
                 )}>
                   Active
                 </span>
              </div>
      ...
      <div className="p-6 bg-surface-container-low border border-white/5 rounded-[2rem] flex items-center gap-6 shadow-inner relative overflow-hidden">
        ...
        <div className="flex-1 space-y-1">
          ...
          <p className="text-sm font-light leading-relaxed max-w-xl">
             ...
             could save you <span className="text-primary font-bold"> 91.88 </span> annually toward your
